...
export function MarketplaceClient() {
  ...
  return (
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 py-10">
      <header>
        <h1 className="text-3xl font-semibold tracking-tight">League Marketplace</h1>
        <p className="mt-2 text-neutral-600">
          Compare public leagues by aggregate performance and top participants. Season-wide bragging rights.
        </p>
      </header>
      ...
      <section className="rounded-2xl border border-neutral-200 bg-white p-4">
        <div className="grid gap-4 md:grid-cols-[180px_220px_1fr] md:items-end">
          ...
          <div className="text-sm text-neutral-600">Showing {sortedLeagues.length} public leagues.</div>
        </div>
      </section>

      {error ? <p className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</p> : null}
      {isLoading ? <p className="rounded-lg border border-neutral-200 bg-white px-4 py-3 text-sm text-neutral-600">Loading marketplace...</p> : null}
      ...
      {!isLoading && sortedLeagues.length > 0 ? (
        <ul className="grid gap-4 md:grid-cols-2">
          {sortedLeagues.map((league, index) => (
            <li key={league.id} className="rounded-2xl border border-neutral-200 bg-white p-5">
              ...
              <p className="mt-1 text-sm text-neutral-600">{league.description ?? "No description yet."}</p>
